...
def init_model(args):
    ...
    print(f'MiniMind: {sum(p.numel() for p in model.parameters()) / 1e6:.2f} M(illion)')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--load_from', default='../model', type=str, help="Model load path（model=torch，=transformers）")
    parser.add_argument('--save_dir', default='out', type=str, help="Model weight directory")
    parser.add_argument('--weight', default='full_sft', type=str, help="（pretrain, full_sft, dpo, reason, ppo_actor, grpo, spo）")
    parser.add_argument('--lora_weight', default='None', type=str, help="LoRA（None，：lora_identity, lora_medical）")
    ...
    parser.add_argument('--inference_rope_scaling', default=False, action='store_true', help="RoPE（4倍，）")
